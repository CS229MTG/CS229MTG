...
def downloadSinglePage(cardNumber, verbose):
	...
	setsoup = str(soup)
	indy = string.rfind(setsoup, '<a href="/sets/')
	setName = setsoup[indy:indy+500]
	indy = string.find(setName,'>')
	setName = setName[indy+1:]
	indy = string.find(setName,'<')
	setName = setName[:indy]
	
	if verbose: print 'Card Set: '+setName
	...
	for item in dataList:
		...
		if len(val)-decimalPlace < 3: 
			if verbose and longverbose: print '\tPrice missing digit:'+ str(key) + '('+convertUnixTimeToHuman(key)+'), was'+str(val)
			val = val + '0'
		val = val.replace('.','')
		val = val.lstrip('0')
		if key in dataMap.keys():
			if verbose and longverbose: print '\tDuplicate! '+ key + '('+convertUnixTimeToHuman(key)+') was '+dataMap[key] +' and is now '+val
		dataMap[key] = val
	if verbose: print 'Discovered '+str(len(dataMap)) + ' data points to be saved.'
	
	#save locally, putting in nulls where needed
	filename = 'cardPriceData/CardData' +missing0s+ str(cardNumber)+'.txt'
	...
	while datetime < currenttime:
		if not str(datetime) in dataMap.keys():
			if verbose and longverbose: print '\tPrice for ' +str(datetime)+'('+convertUnixTimeToHuman(str(datetime))+') not in data!'
		...
		entriesCount+=1
	f.close()
	if verbose: print 'Lines in file: ' +str(entriesCount)+'+2(cardname+setname)'
...
def main(argv):
	...
	cardNumber = argv[1]
	downloadSinglePage(cardNumber, verbose)
